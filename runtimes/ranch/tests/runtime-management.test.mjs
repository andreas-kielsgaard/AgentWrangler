import assert from "node:assert/strict";
import { test } from "node:test";
import { freePort, request, startNodeProcess, stopProcess, waitForJson } from "@agent-wrangler/test-support";

test("Ranch starts independently with identity and health", async () => {
  const port = await freePort();
  const ranch = startNodeProcess(new URL("../server.mjs", import.meta.url), {
    environment: { RANCH_PORT: String(port), AUTHORITY_URL: `http://127.0.0.1:${await freePort()}` },
    label: "ranch",
  });
  try {
    const identity = await waitForJson(`http://127.0.0.1:${port}/identity`);
    assert.equal(identity.runtime.id, "ranch");
    assert.equal((await request(`http://127.0.0.1:${port}/health`)).body.status, "ok");
  } finally {
    await stopProcess(ranch);
  }
});
